...
use backups::backup_item::{BackupCreationMethod, BackupItem, BackupType, HashedBackupItem};
...
pub async fn create_manual_backup(id: web::Path<String>, req: HttpRequest) -> impl Responder
{
	if let Some(user) = req.extensions().get::<User>() {
		...
		if let Some(server_directory) = server.directory {
			let item = match BackupItem::create_backup(id_number, Path::new(&server_directory).to_path_buf(), BackupCreationMethod::MANUAL, 0, BackupType::Full) {
				Ok(b) => b.hash(),
				Err(e) => {
