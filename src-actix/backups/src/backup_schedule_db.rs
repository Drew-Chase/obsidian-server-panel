...
pub fn get(id: u32) -> Result<Option<BackupSchedule>, Box<dyn Error>> {
	...
	if State::Row == stmt.next()? {
		...
		Ok(Some(BackupSchedule::new(id, server, backup_type, interval as u32, exec_if_empty, exec_if_offline)))
	} else {
		Ok(None)
	}
}
...
pub fn list() -> Result<Vec<BackupSchedule>, Box<dyn Error>>
{
	...
	while State::Row == stmt.next()? {
		...
		schedules.push(BackupSchedule::new(id, server, backup_type, interval as u32, exec_if_empty, exec_if_offline));
	}
